...
func (r *Runner) runTF(
	...
	if run.Request.Type == tfaplv1beta1.ForcedUnlock {
		te.forceUnlock(ctx, run.Request.LockID)
	}
	...
	// return if plan only mode or force unlocked a state
	if run.PlanOnly || run.Request.Type == tfaplv1beta1.ForcedUnlock {
